...
def extract_endpoints(js_url):
    ...
    try:
        ...
        url_matches = re.findall(r'(?:(?:https?://|www\.)[^\s/$.?#].[^\s]*)', js_code)
        endpoint_matches = re.findall(r'["\'](/[^"\']{5,})["\']', js_code)
        ...
        if urls:
            for url in urls:
                print(f"[yellow3][-][dark_olive_green1] URL found in [white]{js_url}:[red1] {url}")
                        
        if endpoints:
            for endpoint in endpoints:
                print(f"[yellow3][-][dark_olive_green1] Endpoint found in [white]{js_url}:[red1] {endpoint}")
...
def has_basic_auth(js_url):
    try:
        ...
        if "basic" in js_code.lower():
            base64_matches = re.findall(r'["\'](Y[A-Za-z0-9+/=]+)["\']', js_code)
            for base64_match in base64_matches:
                try:
                    decoded_data = base64.b64decode(base64_match)
                    if decoded_data:
                        print(f"[yellow3][-][dark_olive_green1] AUTHORIZATION BASIC found in [white]{js_url}: [red1]{decoded_data.decode('utf-8')}")
                except:
                    pass
...
def find_api_keys(js_url):
    ...
    try:
        ...
        for api_name, regex_pattern in api_key_patterns.items():
            matches = re.search(regex_pattern, js_code)
            if matches:
                print(f"[yellow3][-][dark_olive_green1] Posible [white]{api_name}[dark_olive_green1] found in [white]{js_url}: [red1]{matches.group(0)}")
...
def find_users_in_js(js_url):
    ...
    try:
        ...
        for username in users_list:
            if username in js_code:
                print(f"[yellow3][-][dark_olive_green1] Posible username or password found in [white]{js_url}: [red1]{username}")
